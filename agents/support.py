...
class AuthenticatedUserNode(MultiRetrievalNode):
    ...
    def _get_default_chain(self):
        template = (
            "You are a helpful assistant. Politely explain that the user’s question "
            "is outside your domain (orders, refunds, shipping, returns). "
            "Invite them to ask something related.\n\n"
            "Human: {input}"
        )
