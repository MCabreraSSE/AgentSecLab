...
import os
from langchain.chat_models import ChatOpenAI          # fallback (OpenAI)
...
class CustomerSupportPipeline:
    ...
    def __init__(self):
        ...
        if os.getenv("OLLAMA_BASE_URL"):
            # ► Usar instancia de Ollama (URL y modelo vienen por env-vars)
            self._llm_model = ChatOllama(
                base_url=os.getenv("OLLAMA_BASE_URL"),
                model=os.getenv("LLM", "mistral"),     # etiqueta de tu modelo
                temperature=0,
            )
        else:
            # ► Fallback a OpenAI si no hay Ollama
